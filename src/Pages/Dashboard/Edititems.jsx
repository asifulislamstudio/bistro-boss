    ...
    const onSubmit = async(data) => {
        ...
        if(res.data.success){
          const updateMenuItem = {
            ...
            const upres = await axiosSecure.patch(`/menu/${item._id}`, updateMenuItem);
            console.log(upres.data);
            if(upres.data.modifiedCount > 0) {
                reset();
                Swal.fire('Recipe Updated')
            }
                        ...
                        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-2 text-sm">
                                    ...
                                    <select  {...register("category", { required: true })} defaultValue={item.category} className="select select-warning">
                            ...
                            <button type="submit" className=" flex  items-center gap-x-2 rounded-md bg-amber-400 px-4 py-2 text-white transition-colors hover:bg-black  "> <ImSpoonKnife></ImSpoonKnife> Update </button>
                        </form>
                    </div>
                </div>
